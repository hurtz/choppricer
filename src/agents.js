...
//
// TWO THINGS FOUND WHILE PORTING HIM THAT ARE NOT HIS, and neither is fixed
// here because fixing either one moves fourteen shoppers:
//
...
//      (rootScale - 1) of its excursion. He divides his speed by his root
//      scale; the crowd does not, and body 11 is carrying 11.4%.
...
//      zero). `poseWalk`'s `pinRoll` flag fixes it for him and is OFF for
//      everyone else, which is why the crowd's numbers below are identical to
//      the digit rather than merely close.
//
...
export function createAgents(THREE, scene, world) {
  ...
  function animateCop(dt, moving, boosted) {
    ...
    // shoppers. The plant is the identity `v*T = 2*S` — the body travels two
    // step lengths per cycle — and it only holds when v and S are in the SAME
    // units. `u.speed` is world metres a second and S comes out of a solve in
    // root-local ones, so the foot's drawn excursion is rootS times what the
    // solve planted and the stance foot skates by (rootS - 1) of it. Measured
    // on him before this line: 74 mm of drift per foot-flat window, on a rig
    // whose solve slip is 0.0 mm.
    //
    // Dividing the speed by the root scale is the whole fix, and it has to be
    // BOTH here and in the phase rate below or the two disagree. THE SAME
    // ERROR IS ON EVERY SHOPPER — 11.4% on the tallest body in the roster —
    // and it is reported rather than fixed, because fixing it changes every
    // shopper's cadence and this round may not move them. See plantCheck().
    const rootS = cop.scale.x || 1;
    const vLocal = u.speed / rootS;
    const stride = K.copStride * (1 - K.copStrideTired * F);
    const step = stepLength(vLocal, L, stride, true);          // heavy: always
    ...
    poseWalk(r, {
      phase: u.phase, speed: vLocal, L, step, duty: dutyOf(vLocal, true) + K.copDuty,
      ...
      hipZAdd: u.lean,
      // He is the only body in the game that gets the roll-aware sole pin, and
      // the flag exists so that the fourteen who do not are provably unmoved.
      pinRoll: 1,
    });
  ...
  function copCheck() {
    ...
    if (Math.abs(unitErr) > 1.0) bad.push('the gait clock and the drawn foot disagree by '
      + unitErr + ' mm a cycle: the solve is in root-local metres and the store is not. '
      + 'animateCop must divide u.speed by cop.scale.x before stepLength() AND before the '
      + 'phase rate, or the stance foot skates by (rootScale - 1) of its own excursion.');
    return {
      ok: bad.length === 0, bad,
      feetOk: !!r.feetOk, kneeOk: !!r.kneeOk,
      kneeCutErrMM: cutErr, rootScale: +rootS.toFixed(4),
      unitErrMMPerCycle: unitErr,
  ...
  function poseWalk(r, o) {
    ...
    // the sole pin needs them: see `pinRoll` below. Nothing between here and
    // their assignment reads or writes any of them, so a shopper gets the same
    // numbers in the same order it always did.
    ...
    if (r.feetOk) {
      if (gz > 0.02) {
        ...
        // second-order term. AT hz = 0 THIS IS EXACTLY THE OLD EXPRESSION, so
        // `pinRoll` unset reproduces the shipped build to the digit — which is
        // what every shopper is still passed, deliberately. The same error is
        // on them (their pivots are narrower and their list smaller, so it is
        // 19 mm at the roster's worst rather than 40) and moving it is a
        // regression this round is not allowed to take. It is reported instead.
        let fL = floorY, fR = floorY;
        if (o.pinRoll) {
          const ch = Math.cos(hz), sh = Math.sin(hz);
          const pin1 = (px, rz) => -((L - _G.drop - (o.sink || 0))
            + (px + L * Math.sin(rz)) * sh) / Math.max(0.5, ch);
          fL = pin1(r.stance0 - sway, legZ + (o.splay || 0));
          fR = pin1(r.stanceR0 - sway, legZ - (o.splay || 0));
        }
  ...
  function animateShopper(s, dt, target) {
    ...
    r.rest0 = 1 - 0.62 * gait;
    poseWalk(r, {
      phase: s.phase, speed: s.speed, L, step, duty: dutyOf(s.speed, heavy),
