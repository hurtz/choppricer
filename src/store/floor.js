...
export function reflectiveFloor(THREE, opts) {
  ...
  const mat = new THREE.MeshStandardMaterial({
    map, color: 0xc4b699, roughness: 0.30, metalness: 0.0,
  });
...
vec2 chopRun( float x ) {
  ...
  return mix( vec2( u, isRun ), vec2( ue, 1.0 ), isEdge );
}
...
float chopLight( vec2 q, float bx, float bz ) {
  ...
  return rows * mix( duty * lamp, uFixDuty * 0.88, merge ) * ext;
}
`)
      .replace('#include <opaque_fragment>', `#include <opaque_fragment>
{
  ...
  if ( ny > 0.0025 ) {
    ...
    for ( int i = 0; i < 6; i ++ ) {
      ...
      vec3 Q = Pw + R * t;
      vec2 rr = chopRun( Q.x );
      if ( abs( rr.x ) < uRunHalf && rr.y > 0.5 ) hitT = min( hitT, t );
    }
    if ( hitT < 1.0e8 ) {
      vec3 Q = Pw + R * hitT;
